...
function App(){
 ...
 return <div className="app">
   ...
   {tab==='settings'&&<main><h2>设置</h2><div className="setting"><b>22:00提醒</b><span>每天晚上提醒今天还没有完成</span><label className="switch"><input type="checkbox" defaultChecked onChange={e=>{if('Notification'in window&&e.target.checked)Notification.requestPermission()}}/><i/></label></div><p className="note">提醒需要系统允许通知。数据只保存在本机，不上传服务器。</p>{active&&<button className="danger" onClick={()=>setModal({type:'abandon',active})}>放弃当前周期</button>}<button className="outline" onClick={newCycle}>新开一个周期</button></main>}
