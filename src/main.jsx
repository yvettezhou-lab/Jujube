...
const encouragements=['今天也稳稳完成啦！','又拿下一次！','很好，今天的任务完成啦。','坚持住，已经越来越近了。','今天也没有落下，棒！','一次一次完成，49次就这样走出来啦。','今日完成 ✓','又向终点走近了一步。','做到了！明天继续。','今天的自己也值得夸一下。','稳稳地走，慢慢就到啦。','很好，今天这一步也完成了。'];
...
function initial(){return{cycles:[],active:null,encUsed:[],settings:{notify:true}}}
function load(){try{return JSON.parse(localStorage.getItem(KEY))||initial()}catch{return initial()}}
function save(s){localStorage.setItem(KEY,JSON.stringify(s))}

function App(){
 const [state,setState]=useState(load); const [tab,setTab]=useState('today'); const [modal,setModal]=useState(null); const notify=state.settings?.notify!==false;
 ...
 function completeToday(){
   ...
   const next={...active,completed:[...active.completed,today()],encIndex:((active.encIndex||0)+1)%encouragements.length};
 ...
 return <div className="app">
   ...
   {tab==='settings'&&<main><h2>设置</h2><div className="setting"><div className="settingText"><b>22:00提醒</b><span>打开「枣」时，会在22:00提醒今天还没有完成</span></div><label className="switch"><input type="checkbox" checked={notify} onChange={async e=>{const on=e.target.checked;if(on&&'Notification'in window&&Notification.permission==='default')await Notification.requestPermission();setState(s=>({...s,settings:{...(s.settings||{}),notify:on}}))}}/><i/></label></div><p className="note">提醒需要允许通知，并且需要「枣」保持在后台可被系统唤醒的范围内。数据只保存在本机，不上传服务器。</p>{active&&<button className="danger" onClick={()=>setModal({type:'abandon',active})}>放弃当前周期</button>}{!active&&<button className="outline" onClick={newCycle}>新开一个周期</button>}</main>}
...
return <div className="shade"><div className="modal cycleModal"><h3>本周期</h3><div className="cycleSummary"><span>已完成 <b>{modal.active.completed.length}/49</b></span><span>中断 <b>{modal.active.interruptions.length} 天</b></span><span>预计 <b>{fmt(addDays(modal.active.start,48+modal.active.interruptions.length))}</b></span></div><div className="timeline">{Array.from({length:49},(_,i)=>{const d=addDays(modal.active.start,i),done=modal.active.completed.includes(d),interrupted=modal.active.interruptions.includes(d);return <div className={"timelineRow "+(done?"done ":"")+(interrupted?"interrupted":"")} key={d}><span className="timelineMark">{done?"✓":interrupted?"✕":"·"}</span><span className="timelineDate">{fmt(d)}</span><span className="timelineStatus">{done?"完成":interrupted?"中断":i===48?"第49次":"待完成"}</span></div>})}</div><button className="outline" onClick={close}>关闭</button></div></div>
}
